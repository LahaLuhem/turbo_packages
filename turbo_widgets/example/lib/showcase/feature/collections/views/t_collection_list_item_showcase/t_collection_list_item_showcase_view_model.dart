...
/// Drives the product detail page for [TCollectionListItem].
///
/// Exposes tunable cells for the list item's text fields and a "last
/// tap fired" observable for callback demonstration.
class TCollectionListItemShowcaseViewModel extends TBaseViewModel<void> {
  TCollectionListItemShowcaseViewModel();
  ...
  final TNotifier<String> _title = TNotifier<String>('List Item');

  final TNotifier<String?> _subtitle =
      TNotifier<String?>('A list-style row for collection sections');
  ...
  /// The title text displayed on the list item.
  ValueListenable<String> get title => _title;

  /// The subtitle text, or `null`.
  ...
  void onTap() => _lastTapFired.value = 'Item tapped';
}
