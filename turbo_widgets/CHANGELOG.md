...
### Added
* `TBentoGrid.multiplierThreshold` — items-per-row parameter for automatic height calculation
* `TBentoGrid.calculatedMinHeight` — minimum height floor for calculated grid dimensions
* Exported `TCategorySectionLayout`, `TContextualAllowFilter`, `TContextualPosition`, `TContextualVariation` enums
